    ...
    public function generateXml()
    {
        ...
        if (is_array($removeInstructions)) {
            foreach ($removeInstructions as $infoNode) {
                ...
                // changes start here
                $ifConfigPath = isset($attributes->ifconfig) ? (string)$attributes->ifconfig : false;
                if ($ifConfigPath && !Mage::getStoreConfigFlag($ifConfigPath)) {
                    continue;
                }

                $unlessConfigPath = isset($attributes->unlessconfig) ? (string)$attributes->unlessconfig : false;
                if ($unlessConfigPath && Mage::getStoreConfigFlag($unlessConfigPath)) {
                    continue;
                }
                // changes end here
